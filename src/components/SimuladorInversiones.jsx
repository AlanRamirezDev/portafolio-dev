...
export default function SimuladorInversiones() {
  ...
  const obtenerPortafolio = async () => {
    try {
      ...
      console.error("Error al conectar con el backend de Java:", error);
    } finally {
      setCargando(false);
    }
  };
  ...
  const inyectarCapital = async () => {
    if (!montoInyeccion || isNaN(montoInyeccion) || Number(montoInyeccion) <= 0) return;
    try {
      await fetch('http://localhost:8080/api/v1/portafolios/1/inyeccion', {
        method: 'POST',
      ...
      console.error("Error al inyectar capital:", error);
    }
  };

  // Función para comprar USDC simulando un tipo de cambio de 18.50
  const comprarUsdc = async () => {
    if (!montoCompra || isNaN(montoCompra) || Number(montoCompra) <= 0) return;
    try {
      await fetch('http://localhost:8080/api/v1/portafolios/1/comprar-usdc', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error("Error al comprar USDC:", error);
    }
  };
  ...
  const reiniciarPortafolio = async () => {
    try {
      ...
      console.error("Error al reiniciar portafolio:", error);
    }
  };
  ...
  return (
    <div className="bg-surface p-8 rounded-2xl border border-white/5 mt-8 shadow-lg relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-[#121212] p-6 rounded-xl border border-white/10 text-center shadow-inner">
          ...
          <p 
            className="text-4xl font-bold mt-3 text-white truncate" 
            title={`$${balances.mxn.toFixed(2)}`}
          >
        ...
        <div className="bg-[#121212] p-6 rounded-xl border border-white/10 text-center shadow-inner">
          ...
          <p 
            className="text-4xl font-bold mt-3 text-accent truncate" 
            title={`${balances.usdc.toFixed(2)} USDC`}
          >
